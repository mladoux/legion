...
var addMode = function(bot, nicklist, config, channel, by, mode, argument, message) {
    ...
    if(argument == config.botName) {
        ...
        if (canVoice(channel)) {
            ...
            names = nicklist.getNames(channel);
            names.forEach(name => {
                if(! nicklist.getVoice(channel, name)) {
                    bot.send('MODE', channel, '+v', name);
                    nicklist.setVoice(channel, name, true);
                }
            });
        }
...
var remMode = function (bot, nicklist, config, channel, by, mode, argument, message) {
